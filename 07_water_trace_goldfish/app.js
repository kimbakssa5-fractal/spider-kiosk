...
(function () {
  ...
  const ASSET_VER = "57";   // 금색 코이: 4---.png 중앙(직선)포즈 기반 재생성 — 좌턴/중앙/우턴, 꼬리 찢어짐 없음
  const BG_VER = "4";
  ...
  //   겹침 구간을 양쪽에 복제(wallU 접힘) + 코사인 그라데이션(합=1) → 이음새 소거.
  ...
  if (helpEl) {
    ...
    helpEl.addEventListener("click", function () { helpEl.classList.add("hidden"); });   // 아무데나 터치=닫기
  }
  function toggleHelp() {
    if (!helpEl) return;
  ...
  // I 키: 중앙 손가락 아이콘(힌트) 숨김 토글 — 기본 숨김(index.html #hint 에 icon-off 클래스)
  let hintIconHidden = true;
